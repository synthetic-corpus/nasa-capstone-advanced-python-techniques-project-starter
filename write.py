...
output of the `limit` function and the filename supplied by the
user at the command line. The file's extension determines which of
...
def prep_csv(close_approach):
    ...
    It takes in one close approach object and formats its for
    CSV dict injest"""
    return_this = {}
    ...
    for key, value in tempt_dict.items():
        if value is None:
            value = ''  # CSV files require empty strings
        if value is False:
            value = 'False'
        if value is True:
            value = 'True'
...
def write_to_json(results, filename):
    ...
    output is a list containing dictionaries, each mapping `close_approach`
